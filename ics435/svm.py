...
def makenu():
    X1, Y1, o, p, q = polys(20)
    #print(str(o) + ' ' + str(p) + ' ' + str(q))
    
    gaussian_noise_train = np.random.normal(scale=0.1, size=2)
    #gaussian_noise_test = np.random.normal(scale=0.3 , size=2)
    print("gaussian_noise_train: ",gaussian_noise_train)
  ...
  #print(X1)
    ...
    clf = svm.NuSVC(nu=0.1, degree=3,kernel='poly')
...
def test_nu():
    ...
    noise_array = np.arange(0.01, 0.2, 0.01)
